...
Lista* catord(Lista *a,Lista*b){
    ...
    while(x<a->FIM || y<b->FIM){
        ...
        if(a->vet[x]<b->vet[y] && x<a->FIM && y<b->FIM){
            insere_elem(c,a->vet[x++]);

        }
        else if(b->vet[y]<a->vet[x] && x<a->FIM && y<b->FIM){
            insere_elem(c,b->vet[y++]);
        }
        if(x==a->FIM)insere_elem(c,b->vet[y++]);
        if(y==b->FIM)insere_elem(c,a->vet[x++]);
    }
